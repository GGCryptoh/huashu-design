 ...
 *   - Text becomes real text frames — double-click to edit in PowerPoint
 *   - body size 960pt × 540pt (LAYOUT_WIDE, 13.333″ × 7.5″)
 *
 * ⚠️ The HTML must satisfy 4 hard constraints (see references/editable-pptx.md):
 *   1. Text lives inside <p>/<h1>-<h6> (a <div> can't hold text directly)
 *   2. No CSS gradients
 *   3. <p>/<h*> can't have background/border/shadow (put those on a wrapper div)
 *   4. <div> can't use background-image (use <img>)
 *
 * Visually-driven HTML almost never passes — write the HTML to these constraints from line 1.
 * For cases where visual freedom matters more (animation, web components, CSS gradients,
 * complex SVG), use export_deck_pdf.mjs / export_deck_stage_pdf.mjs and export to PDF instead.
 *
 * Deps: npm install playwright pptxgenjs sharp
 *
...
function parseArgs() {
  ...
  if (!args.slides || !args.out) {
    ...
    console.error('   For visually-driven cases, use export_deck_pdf.mjs to export PDF instead.');
    process.exit(1);
  }
  return args;
}
...
async function main() {
  ...
  try {
    ...
    console.error(`  If deps are missing, run: npm install playwright pptxgenjs sharp`);
    process.exit(1);
  }
  ...
  if (errors.length) {
    console.error(`\n⚠️ ${errors.length} slide(s) failed to convert. Common cause: HTML doesn't meet the 4 hard constraints.`);

...
<p align="center"><sub>
  ▲ 25 sec · Terminal → 4 directions → Gallery ripple → 4 Focus passes → Brand reveal<br>
  👉 <a href="https://www.huasheng.ai/huashu-design-hero/">Visit the interactive HTML version (with audio)</a> ·
  <a href="https://github.com/alchaincyf/huashu-design/releases/download/v2.0/hero-animation-v10-en.mp4">Download MP4 (BGM + SFX · 10MB)</a>
</sub></p>
...
Mandatory whenever the task involves a specific brand: ask → search → download (three fallback paths) → grep colors → write `brand-spec.md`.
...
## Showcase · Real-world cases

### "Let's Talk Skill" · PM after-party keynote deck

> **Live demo · [https://skill-huasheng.vercel.app](https://skill-huasheng.vercel.app)**

13-slide HTML deck, **entirely built with huashu-design**:
...
- Each cinematic shows a **full static workflow dashboard by default** (audience can always see how the skill runs); pressing ▶ triggers the animation, which fades back to the dashboard when done
- Embedded huasheng.ai's 25-second hero animation (with iframe local fallback)
- Real data: 14,495 stargazers' real curve (pulled from gh API) + DeepSeek V4 real specs (verified via WebSearch)
- Real AI assets: a 4×2 grid generated with `huashu-gpt-image`, then `extract_grid.py` cuts out 8 individual transparent PNGs for the 3D orbit float

**Pages worth referencing**:
- `/slides/slide-04b-nuwa-flow.html` · static dashboard + cinematic overlay double-layer architecture
- `/slides/slide-06b-darwin-flow.html` · contrasting case in a fully independent visual language
...
| 3 · Download assets | SVG file → full HTML of official site → product screenshot color pick | Three fallback paths — if one fails, immediately try the next |
| 4 · grep color values | Pull every `#xxxxxx` from assets, sort by frequency, filter out black/white/gray | **Never guess brand colors from memory** |
| 5 · Freeze the spec | Write `brand-spec.md` + CSS variables; all HTML references `var(--brand-*)` | Un-frozen knowledge evaporates |
...
│   ├── html2pptx.js
│   └── verify.py
└── demos/                   # 9 capability demos (c*/w*) as HTML + hero v10
```
...
| WeChat Official Account | Huashu | Search "花叔" in WeChat |

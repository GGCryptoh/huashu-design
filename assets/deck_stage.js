/**
 * <deck-stage> — HTML slide-deck shell, as a web component
 *
 * Features:
 ...
 * - Speaker notes via postMessage (so an outer page can render them)
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render shadow DOM first (independent of children, unaffected by parser timing)
      this._render();

      // Defensive: if the script is in <head> (rather than after </deck-stage>),
      // the parser may not yet have processed the child <section>s, so
      // querySelectorAll would return empty. Defer to the next event-loop tick
      // so all children are guaranteed parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        ...
        // Document already parsed (script at end of body or deferred); collect on next frame
